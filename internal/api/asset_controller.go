...
func (ct *AssetController) GetAssetRvalue(c *gin.Context) {
	...
	if err != nil {
		ginlogrus.GetCtxLogger(c).Error(err)
		c.Error(err)
		return
	}
...
func (ct *AssetController) GetAssetSignature(c *gin.Context) {
	...
	logger.Error(dlcData.PublishedDate)
	logger.Error("IS IT SIGNED?")
	if !dlcData.IsSigned() {
		logger.Error("IS NOT SIGNED")
		...
		decomposedValue := decompose.DecomposeValue(roundedValue, ct.config.SignConfig.Base, ct.config.SignConfig.NbDigits)
		...
		logger.Error(dlcData.PublishedDate)
		logger.Error(dlcData.Signatures)
		logger.Error(sigs)
		...
		logger.Error(dlcData.PublishedDate)
		logger.Error(len(dlcData.Signatures))
		if err != nil {
			logger.Error("ERROR")
